...
handler.all = async function (m, { isPrems }) {
    ...
    if (/^.*tiktok/i.test(m.text)) {
        let res = await fetch(API('alpha', '/api/tiktok', { url }, 'apikey'))
        if (!res.ok) return m.reply(eror)
        let json = await res.json()
        await m.reply(wait)
        // m.reply(util.format(json))
        await this.sendFile(m.chat, json.results.nowm, '', '© Liliana Bot', m)
    }
    ...
    if (/^.*cocofun/i.test(m.text)) {
        ...
        await m.reply(wait)
        // m.reply(util.format(json))
        await this.sendFile(m.chat, json.download, '', '© Liliana Bot', m)
    }

    if (/^.*(fb.watch|facebook.com)/i.test(m.text)) {
        let res = await fetch(API('lol', '/api/facebook', { url }, 'apikey'))
        if (!res.ok) return m.reply(eror)
        let json = await res.json()
//        if (!json.status) return m.reply(util.format(json))
        await m.reply(wait)
        await conn.sendFile(m.chat, json.result, '', `© Liliana Bot`, m)
    }


if (/^.*(instagram.com)/i.test(m.text)) {
        let res = await fetch(API('lol', '/api/instagram', { url }, 'apikey'))
        if (!res.ok) return m.reply(eror)
        let json = await res.json()
//        if (!json.status) return m.reply(util.format(json))
        await m.reply(wait)
    ...
    if (/^.*(pinterest.com\/pin|pin.it)/i.test(m.text)) {
        pin(url).then(async res => {
            ...
            await m.reply(wait)
            m.reply(util.format(json))
            await this.sendFile(m.chat, json.data.url, '', '© Liliana Bot', m)
        }).catch(_ => _)
    }
    ...
    if (/^.*twitter.com\//i.test(m.text)) {
        twitter(url).then(async res => {
            ...
            for (let { url } of json.data) {
                this.sendFile(m.chat, url, 'ig' + (/mp4/i.test(url) ? '.mp4' : '.jpg'), '© Liliana Bot', m)
            }
        }).catch(_ => _)
    }
...
`.trim(), '© Liliana Bot', 'Audio', `.yta ${vid.url}`, 'Video', `.yt ${vid.url}`)
    }*/
